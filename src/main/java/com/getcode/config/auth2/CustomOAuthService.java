...
import static com.getcode.domain.member.SocialType.KAKAO;
import static com.getcode.domain.member.SocialType.NAVER;
...
public class CustomOAuthService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        OAuthAttribute extractAttributes = OAuthAttribute.of(socialType, userNameAttributeName, attributes);

        Member createdMember = getUser(extractAttributes, socialType); // getUser() 메소드로 User 객체 생성 후 반환

        // DefaultOAuth2User를 구현한 CustomOAuth2User 객체를 생성해서 반환
        return new CustomOAuth2User(
                Collections.singleton(new SimpleGrantedAuthority(createdMember.getAuthority().toString())),
                attributes,
